console.log("entra en el script de js");
...
function jugadaSeleccionada() {
  ...
  // console.log("Seleccionaste la ficha: ", Array.isArray(valorBoton) ? valorBoton.join(", ") : valorBoton);
}
...
btnJugar.addEventListener("click", () => {
  ...
  if (totalFichas < 1) {
    mostrarNotificacion("No tienes fichas suficientes para jugar. Por favor, reinicia el juego.");
    return;
  }
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        ...
        console.error("Error al obtener el número aleatorio:", xhr.statusText);
        mostrarNotificacion("Error al obtener el número aleatorio. Por favor, inténtalo de nuevo.");
      }
    }
  };
